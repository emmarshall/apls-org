---
pagetitle: "2026 American Psychology-Law Society Conference" 
toc: false
css: index.css
editor: source
description: | 
  The annual conference for the American Psychology-Law Society for 2026 will be held in Reno, NV from March 12-14th.
includesource: true
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
source("../assets/apls_functions.R")

```

:::::: column-screen
::::: hero-banner
:::: content-block
### 2026 Annual Conference of the American Psychology-Law Society

#### {{< fa location-dot aria-hidden="true" >}} Reno, Nevada

#### {{< fa calendar aria-hidden="true" >}} March 12-14, 2026

::: hero-buttons
[Learn More](#register){.button-17 .btn-guide role="button"} 
:::
::::
:::::
::::::

::: column-screen
```{r}
#| label: TOC
#| echo: false
#| warning: false
#| message: false

content = rowwise_table(~name, ~icon, ~link,
  "General", "info-circle", "#info",             
  "Registration", "person-add", "#registration",
  "Hotel", "airplane", "#hotel",
  "Program", "person-workspace", "#program",
  #"Registration", "person-badge", "#register",  
  "Archives", "journal-text", "archive/index.qmd"
)
div(class = "bg-light",
 div(class = "bg-breakout",
    div(class = "inner-container",
      div(class = "boxed-wrapper",
            div(class = "boxed-feature-grid",
          pmap(content, function(name, icon, link) {
            div(class = "boxed-feature-grid__item",
              div(class = "boxed-feature-grid__icon",
                a(href = link, class = "boxed-feature-grid__link",
                  tags$i(class = sprintf("bi-%s boxed-feature-grid__i", icon))
                )),
    div(class = "boxed-feature-grid__title", name)
)
          })
        )
      )
    )
  )
)

```
:::

# General Information {#info}

<figure class="text-end">

<blockquote class="blockquote">

<p class="mb-0">

If you have any questions or comments about the conference, please contact the conference co-chairs at [conference\@ap-ls.org](mailto:conference@ap-ls.org) <br>

Looking forward to seeing everyone in March!

<figcaption class="blockquote-footer">

2026 AP-LS Conference Co-Chairs <cite title="Luna Filipović, *Ph.D. Cantab* & Heath Hodges, *Ph.D., M.L.S., ABPP (Forensic)*">Luna Filipović, *Ph.D. Cantab* & Heath Hodges, *Ph.D., M.L.S., ABPP (Forensic)*</cite>

</figcaption>

</figure>

::: {.panel-tabset .box-shadow}
## {{< fa circle-info aria-hidden="true" >}} Welcome!

![](../images/2026_announcement.png)
:::

::: vspace
:::

::::: column-screen
:::: {.simple-info-blue .alt-background-blue}
::: content-block-blue
##### Want to learn more about the [AP-LS Annual Conference]{.highlight}?

[Learn More {{< fa arrow-right aria-hidden="true" >}}](archive/index.qmd){.btn-action .btn .btn-secondary .btn-lg role="button"}
:::
::::
:::::

::: vspace
:::

# Registration {#register}

:::::::: simple-info-center
::::::: {.panel-tabset .box-shadow}
## {{< fa bullhorn >}} Info

#### {{< fa id-badge aria-hidden="true" >}} Online registration is open on December 1st!

::: vspace
:::

[*If you have any issues registering, please email <br> {{< fa envelope title="Email" >}} {{< var email.office >}}*]{.session-chair}

::: vspace
:::

[Click Here {{< fa arrow-right-long aria-hidden="true" >}}]( https://apls.memberclicks.net/apls2026conference#){.btn .btn-outline-primary .ml-auto role="button"}

::: vspace
:::

## {{< fa circle-info >}} Pricing

::: {.callout-note appearance="simple"}
## REGISTRATION RATES & INCLUSIONS

+-----------------------+-----------------+--------------+
| Status                | Early Bird\     | Regular\     |
|                       | (before Jan 31) | (on Feb 1)   |
+:======================+:================+:=============+
| AP-LS Full Members    | \$345           | \$380        |
+-----------------------+-----------------+--------------+
| AP-LS ECP Members     | \$240           | \$270        |
+-----------------------+-----------------+--------------+
| Non Members           | \$490           | \$575        |
+-----------------------+-----------------+--------------+
| AP-LS Student Members | \$90            | \$125        |
+-----------------------+-----------------+--------------+
| Non Members Students  | \$170           | \$210        |
+-----------------------+-----------------+--------------+
:::
:::::::
::::::::

<hr>

::: vspace
:::

# Program & Schedule{#program}

:::::: simple-info-center
::::: {.panel-tabset .box-shadow}
## {{< fa circle-info aria-hidden="true" >}} Overview

::: vspace
:::

##### [2026 APLS ***Draft*** Program Grid]{.highlight} <br> is now available!


[Download {{< fa arrow-right aria-hidden="true" >}}](https://drive.google.com/file/d/1SDyFn-jD_2yDOrjMBh2b9lu8JIoPaCMb/view?usp=sharing){.btn .btn-outline-primary role="button"} 

::: vspace
:::

## {{< fa bullhorn aria-hidden="true" >}} Preconference Workshops

::: vspace
:::

##### [{{< fa calendar aria-hidden="true" >}} Wednesday, March 11, 2026]{.highlight}


::: vspace
:::

[*Attendees will have the ability to register for the pre conference workshops with their conference registration*]{.session-chair}

::: vspace
:::

[Learn More {{< fa arrow-right-long aria-hidden="true" >}}](preconference/index.qmd){.btn .btn-outline-primary .ml-auto role="button"}

::: vspace
:::

:::::
::::::

<hr>

::: vspace
:::

# Hotel Information {#hotel}

::::::: simple-info
:::::: {.panel-tabset .box-shadow}
## {{< fa hotel aria-hidden="true" >}} Peppermill Resort Spa Casino

![](https://images.bestwestern.com/bwi/brochures/29095/photos/1024/25995732.jpg){fig-align="center" fig-alt="Exterior view of the Peppermill Resort Spa Casino"}

<center>[{{< fa map aria-hidden="true" >}} Plan your trip](https://www.worldhotels.com/content/luxury-hotels/en_US/book/hotels-in-reno/peppermill-resort-spa-casino/propertyCode.29095.html){.btn .btn-outline-primary role="button"} [{{< fa location-dot aria-hidden="true" >}} Map](){.btn .btn-outline-primary role="button"}</center>

::: vspace
:::

[Conference Rates]{.plenary}

:::: session-description
Attendees can also call the Peppermill to make their reservation. They just need to mention your group code to receive your group rate and waived resort fee.

::: callout-important
The 2026 room block is now closed!!  People can still book directly through the [hotel at this link](https://www.peppermillreno.com/).
:::

Group code: `CAPLS26`

::: hero-buttons
[{{< fa bell-concierge aria-hidden="true" >}} Book a Room](https://book.passkey.com/event/51002110/owner/7268/home){.btn-action .btn .btn-action .ml-auto role="button"} [{{< fa hotel aria-hidden="true" >}} Website](https://www.worldhotels.com/content/luxury-hotels/en_US/book/hotels-in-reno/peppermill-resort-spa-casino/propertyCode.29095.html){.btn-action .btn .btn-action-primary .ml-auto role="button"}
:::
::::
::::::
:::::::
